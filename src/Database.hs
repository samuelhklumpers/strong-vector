...
import SingBase
import Tensors
import Naturals (Length, toFin)
import Data.Type.Bool
import qualified Data.Type.Equality as T
import NaturalsFams
import NaturalsFuns (sizeList)

-- TODO: Swap insert row or columns
...
data Table c r = Table (HList  c) (Tensor (Length c ': r ': '[]) String) deriving (Show)
...
instance Show (HList h) where
    show XNil                   = ""
    show (XCons x xs)           = "-" ++ show xs

data Header :: [C] -> N -> * where
  Header ::  Nat a -> Header n a

class Member string xs t head where
  select' :: STRING s -> HList xs -> t
...
select :: forall s ss. Member s ss (Nat (Lookup s ss)) (Head s ss) => STRING s -> HList ss -> Nat (Lookup s ss)
...
restFin :: Nat n -> Nat m -> Nat (m - n)
restFin NZ s            = s
restFin _ NZ            = NZ
restFin (NS s1) (NS s2) = restFin s1 s2
...
selectByIndex :: (Length c ~ 'S m, (n + 'S (m - n)) ~ 'S m) =>
            Nat n -> Table c r -> Tensor '[r] String
selectByIndex x (Table _ y) = getColumn y (toFin x (restFin x (sizeT y)))
  where getColumn :: Tensor (n ': r ': '[]) String -> Fin n -> Tensor (r ': '[]) String
        getColumn ((TC (VC v _)))  FZ            = v
        getColumn ((TC (VC _ (VC v vs)))) (FS s) = getColumn (TC (VC v vs)) s

-- | Select a column from a table by giving the name of the column
selectFromTable :: (Member s c (Nat (Lookup s c)) (Head s c),
 (Lookup s c + 'S (m - Lookup s c)) ~ 'S m, Length c ~ 'S m) =>
  STRING s -> Table c r -> Tensor '[r] String
selectFromTable colName t@(Table c _) = selectByIndex (select colName c) t

-- TESTING
selectTest = selectFromTable ca addR3
...
addC3 = insertColumn cc newCol addR2
